...
" ---------------------------------------------------------------------------
  ...
  " XXX TODO FIXME just a short HACK for snapshot views ...!
      let l:sfile = fnamemodify(l:fname, ":t:r" )
      let l:fextension = fnamemodify(l:fname, ":e")
      let l:difffile = l:sfile . ".v" . l:cmp_to_ver . "." . l:fextension
      let l:difffile = substitute(l:difffile, "/", "-", "g")

      let l:getcmd = "!cleartool get -to \"" . l:difffile . "\" \"" . l:fname .  "@@" . l:cmp_to_ver . '"'
      exe l:getcmd
      if (v:shell_error)
	echohl Error
	echomsg "CCASE get failed: " . l:getcmd
	echomsg "did the file to diff already exist?"
	echohl None
      else
	echomsg "CCASE get done: " . l:getcmd
      endif

      exe l:splittype . l:difffile

      " Get rid of temp file
      if has('unix')
      silent exe "!rm ".l:difffile
      else
      silent exe "!del ".l:difffile
      endif
...
if has("unix")
  ...
  com! -nargs=0 -complete=command Ctxlsv exec "!LC_ALL=POSIX xlsvtree ".expand("%")." &"
  com! -nargs=0 -complete=command Ctdiff exec
        \ "!LC_ALL=POSIX cleartool diff -graphical -pred ".expand("%")." &"
